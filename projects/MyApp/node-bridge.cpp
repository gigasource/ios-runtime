...
t_bridge_callback node_callback = NULL;
...
napi_value Method_RegisterChannel(napi_env env, napi_callback_info info) {
 ...
 napi_value Method_SendMessage(napi_env env, napi_callback_info info) {
  ...
  NAPI_ASSERT(env, node_callback, "No callback is set in native code to receive the message.");
  if (node_callback) {
    node_callback(channel_name_utf8, msg_buf);
  }
  return nullptr;
}
